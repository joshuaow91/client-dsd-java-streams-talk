...
export default function CoreOperationsOfStreams() {
  return (
    <div className="max-w-7xl mx-auto mt-8 md:mt-16 pb-24 md:pb-48">
      ...
      <p className="text-lg md:text-2xl mb-6 leading-8 md:leading-10">
        Intermediate operations are essential for building a stream pipeline.
        They transform the stream, filter data, or map elements to new values,
        but do not themselves trigger processing. Because these operations are
        lazy, they are evaluated only when a terminal operation is invoked,
...
`}
      ...
      <ul className="list-disc marker:text-pink-500 list-inside text-lg md:text-2xl mb-6 leading-8 md:leading-10">
        <li>
          <span className="text-blue-300 font-bold">Terminal Operations</span>:
          Operations like{" "}
          <span className="text-orange-300 font-bold">collect()</span> and{" "}
          <span className="text-orange-300 font-bold">forEach()</span> produce a
          result or a side effect and trigger the processing of the stream.
        </li>
      </ul>
      <p className="text-lg md:text-2xl mb-6 leading-8 md:leading-10">
        Terminal operations are the final steps in a stream pipeline. They
        initiate the processing of data and produce a result, whether it&apos;s
        a collection of results, a sum, or a side effect like printing to the
        console. Once a terminal operation is invoked, the stream is considered
...
`}
      ...
      <ul className="list-disc marker:text-pink-500 list-inside text-lg md:text-2xl mb-6 leading-8 md:leading-10">
        <li>
          ...
          <span className="text-orange-300 font-bold">anyMatch()</span> can stop
          further processing as soon as a certain condition is met, improving
          efficiency.
        </li>
      </ul>
      <p className="text-lg md:text-2xl mb-6 leading-8 md:leading-10">
        Short-circuit operations are particularly useful for improving the
        efficiency of stream processing. They allow the pipeline to terminate
        early if a certain condition is met, preventing unnecessary computation
